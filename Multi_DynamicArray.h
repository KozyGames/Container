...
namespace Kozy {
	namespace Containers {
		...
		public:
			...
			/** CONSTRUCTOR **/
			Multi_DynamicArray(unsigned typesCap, unsigned typesDynRate, unsigned arrayCap, unsigned arrayDynRate) :beginIterator{ *this,0 }, endIterator{ *this, 0 } {
				...
				if (!arrayCap)
					throw std::invalid_argument("The Capacity of ArrayGroups has to be at least 1.");
#endif
			...
			Multi_DynamicArray() :beginIterator{ *this,0 }, endIterator{ *this, 0 } {
				arrayCapacity = new unsigned int[typesCapacity];
			...
			T** addElementsOfSameGroup(T** arrayPtr, unsigned int count) {
				...
				if (!count) return arrayPtr;
					...
					for (unsigned int index{ currentArraySize[group] }, index2{ 1 }; index2 != count; ++index)
						pointerToArrayGroup[group][index] = arrayPtr[index2++];
...
#ifndef _MULTI_DYNAMIC_NO_CHECK_
				if (!value) throw std::invalid_argument("0 is not a valid argument!");
				else
#endif
...
#ifndef _MULTI_DYNAMIC_NO_CHECK_
				if (!value) { 
					throw std::invalid_argument("0 is not a valid argument!");
					return;
				}
#endif
			...
			unsigned int getElementPositionOrAdd(T& element) {
				...
				for (unsigned int index{ 0 }; index != currentGroupTypesSize; ++index)
					if (element.isSameGroup(*pointerToArrayGroup[index][0])) {
						for (index2 = 0; index2 != currentArraySize[index]; ++index2)
			...
			unsigned int getElementPosition(T& element) {
				unsigned int index2{ 0 };
				for (unsigned int index{ 0 }; index != currentGroupTypesSize; ++index)
					if (element.isSameGroup(*pointerToArrayGroup[index][0])) {
						for (index2 = 0; index2 != currentArraySize[index]; ++index2)
							if (element == *pointerToArrayGroup[index][index2])
								return index2;
						throw std::range_error("Element is not inside the Multi_array!");
						return index2;
					}
			...
			public:
				friend class ArrayIterator;
				friend class Multi_DynamicArray;
				class ArrayIterator {
				public:
					friend class GroupsIterator;

					ArrayIterator(Multi_DynamicArray& ref, unsigned int in = 0, unsigned int in2 = 0) :multiRef{ ref }, index{ in }, index2{ in2 } {}
					...
					T* operator->() {
						return multiRef.pointerToArrayGroup[index][index2];
					}
					T& operator*() {
						return *multiRef.pointerToArrayGroup[index][index2];
					}
					...
					ArrayIterator end() {
						return ArrayIterator(multiRef,index, multiRef.currentArraySize[index]);
					}
					bool operator!=(const ArrayIterator& iter) {
						return index2 != iter.index2;
					}
					bool operator==(const ArrayIterator& iter) {
						return index2 == iter.index2;
					}
				...
				GroupsIterator(Multi_DynamicArray& ref, unsigned int pos) : multiRef{ ref }, arrayIter{ ref }, arrayIterEnd{ ref } {}
				GroupsIterator& operator++() {
					arrayIterEnd.index = ++arrayIter.index;
					arrayIter.index2 = 0;

					arrayIterEnd.index2 = multiRef.currentArraySize[arrayIter.index];
					return *this;
				}
				...
				bool operator!=(const GroupsIterator& iter) {
					return arrayIter.index != iter.arrayIter.index;
				}
				bool operator==(const GroupsIterator& iter) {
					return arrayIter.index == iter.arrayIter.index;
				}
				...
				GroupsIterator& newPosition(unsigned int pos) {
					arrayIterEnd.index = arrayIter.index = pos;
					arrayIter.index2 = 0;
					arrayIterEnd.index2 = multiRef.currentArraySize[arrayIter.index];
			...
			GroupsIterator& begin() {
				return beginIterator.newPosition(0);
			}
			GroupsIterator& end() {
				return endIterator.newPosition(currentGroupTypesSize);
			}
			...
			*/
			void removeArrayGroup(unsigned int arrayGroup) {
#ifndef _MULTI_DYNAMIC_NO_CHECK_
				if (arrayGroup >= currentGroupTypesSize)
					throw std::out_of_range("Invalid arrayGroup");
#endif
			...
			/*
			* Does not delete Elements
			* arr needs to retain all elements
			*/
			void removeAndGetArrayGroup(unsigned int arrayGroup, T** arr, unsigned int arrPosition=0) {
#ifndef _MULTI_DYNAMIC_NO_CHECK_
				if (arrayGroup >= currentGroupTypesSize)
					throw std::out_of_range("Invalid arrayGroup");
#endif
				unsigned int elementsToRemove = currentArraySize[arrayGroup];
				while (elementsToRemove != 0)
					arr[arrPosition++] = pointerToArrayGroup[arrayGroup][--elementsToRemove];
		...
		private:
			...
			GroupsIterator beginIterator;
			GroupsIterator endIterator;
